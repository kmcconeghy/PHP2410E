...
df_mds %>%
  ...
  kable(., digits=1)
```
Where 1 is native american, 2 is asian, 3 is black, 4 is hispanic and 5 is white.  
...
df_mds_dups <- df_mds_2 %>%
  group_by(id) %>%
  summarize(n = n(),
            `Dup. MDS gender` = n_distinct(idgendr, na.rm=T),
            `Dup. MDS race cat.`  = n_distinct(idrace, na.rm=T),
            `Dup. MDS race recode`  = n_distinct(rc_recode, na.rm=T),
            `Dup. MDS race white` = n_distinct(rc_white, na.rm=T),
            `Dup. MDS race black` = n_distinct(rc_black, na.rm=T),
            `Dup. MDS race hispanic`  = n_distinct(rc_hisp, na.rm=T), 
            `Dup. MDS DOB` = n_distinct(idbirdt, na.rm=T),
            `Dup. MDS Birth Year-Month` = n_distinct(paste0(month(idbirdt), '-', 
                                                            year(idbirdt)), na.rm=T)
            ) %>%
  mutate_at(vars(contains('MDS')), factor)
...
df_denom_dups <- df_denom %>%
  group_by(id) %>%
  summarize(`No. records` = n(),
            `Dup. Denom. Gender` = n_distinct(hksex, na.rm=T),
            `Dup. Denom. Race`  = n_distinct(hkrace, na.rm=T),
            `Dup. Denom. DOB` = n_distinct(hkdob, na.rm=T)
            ) %>%
  mutate_at(2:5, factor)

summary(df_denom_dups[,2:5]) %>%
  kable(., digits=2, format.args=list(big.mark = ','))
```
